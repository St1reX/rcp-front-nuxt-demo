    ...
    cell: ({ row }) =>
      h(
        UButton,
        {
          ...
          onClick: () => deleteAttendance(row.getValue('id')),
        },
        () => 'Usuń'
      ),
  },
...
const deleteAttendance = async (attendanceId: string) => {
  try {
    await api.attendance.deleteEmployeeAttendance({
      attendanceId: attendanceId,
    });

    dataForTable.value = dataForTable.value.filter(
    ...
    toast.add({
      title: 'Sukces',
      description: 'Rekord obecności został usunięty.',
      color: 'primary',
    });
  } catch (e: any) {
    useFetchErrorHandler('Obecność', e);
  }
};
  ...
  try {
    ...
    for (let i = 0; i < attendance.value.length; i++) {
      const attendanceRecord: Attendance = {
        ...
        action:
          attendance.value[i].metadata.action === 'EXIT' ? 'Wyjście' : 'Wejście',
      };
...
const seeMonthlyReport = () => {
  router.push({
    path: '/dashboard/attendance/monthly-report',
    query: {
      employeeId: selectedEmployeeId.value,
      year: currentDate.value.getFullYear().toString(),
...
const addAttendance = () => {
  router.push({
    path: '/dashboard/attendance/add-attendance',
    query: {
      employeeId: selectedEmployeeId.value,
      cardKey: employees.value.find(
        (employee) => employee.id === selectedEmployeeId.value
      )?.cards[0].key,
      employeeFirstName: employees.value.find(
        (employee) => employee.id === selectedEmployeeId.value
      )?.firstName,
      employeeLastName: employees.value.find(
        (employee) => employee.id === selectedEmployeeId.value
      )?.lastName,
    },
  });
  ...
  <div class="space-y-8">
    ...
    <div class="flex justify-between">
      ...
      <div class="flex items-center gap-2" v-if="selectedEmployeeId">
        ...
        />
        <UButton label="Dodaj obecność" color="success" @click="addAttendance" />
      </div>
    </div>
    ...
    <div
      v-else-if="selectedEmployeeId && !isLoading"
